...
bool Logger::LogWhenFalse(bool rst, const string &msg)
{
    if (rst == false)
        spdlog::info(msg);
    return rst;
...
bool Logger::LogWhenFalse(bool rst, spdlog::level::level_enum level, const string &msg)
{
    if (rst == false)
...
{
    return_false(Logger::LogWhenFalse(curl != nullptr, level::critical, "curl handler not exist"));
    curl_easy_reset(curl);
    return Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_URL, url.c_str()) == CURLE_OK, level::err, "setting url error") &&
           Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_TCP_KEEPALIVE, 1L) == CURLE_OK, "setting curl error(keepaliave)") &&
           Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_TCP_KEEPIDLE, 120L) == CURLE_OK, "setting curl error(keepidle)") &&
           Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_TCP_KEEPINTVL, 60L) == CURLE_OK, "setting curl error(keepintvl)") &&
           Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L) == CURLE_OK, "setting curl error(follow)");
}
...
bool QuickCurl::ConnectionTest()
{
    ...
    return_false(Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_RANGE, "0-1") == CURLE_OK, "setting curl error(range)"));
    return_false(Logger::LogWhenFalse(curl_easy_perform(curl) == CURLE_OK, level::err, "perform curl error"));
    long response_code;
    curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &response_code);
    spdlog::log(level::trace, fmt::format("response_code: {}", response_code));
    return_false(Logger::LogWhenFalse(response_code == 206, level::err, fmt::format("test connection: {} can't connected.\nresponse_code: {}", url, response_code)));
...
bool QuickCurl::FetchRange(FallbackFn &&fallback, const string &sta, const string &end)
{
    ...
    return_false(ReInitCurl());
    return_false(Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_RANGE, range.c_str()) == CURLE_OK, "setting curl error(range)"));
    return_false(Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_WRITEDATA, &fallback) == CURLE_OK, "setting curl error(fallback function)"));
    return_false(Logger::LogWhenFalse(curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION,
                                      ...
                                      "setting curl error(fallback middle function)"));
    return_false(Logger::LogWhenFalse(curl_easy_perform(curl) == CURLE_OK, level::err, "perform curl error"));
    long response_code;
    curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &response_code);
    spdlog::log(level::trace, fmt::format("response_code: {}", response_code));
